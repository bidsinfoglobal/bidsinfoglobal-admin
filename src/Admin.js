...
export default function Admin() {
  ...
  const [breadcrumb, setBreadcrumb] = useState([...generateBreadCrumb(location.pathname)])
  ...
  function generateBreadCrumb(pathname) {
    ...
    return paths.map((path, index) => {
      ...
      if (index === 0) {
        return { title: <Link key={index} to="/" activeclassname="active" className=''>Dashboard</Link> }
      }
      ...
      if (paths.length == index + 1) {
        return { title: <span className='capitalize'>{searchParams.get('id') && path === 'add' ? 'edit' : path}</span> }
      }
      else
        return { title: <Link key={index} to={url} className='capitalize'>{path}</Link> };
    });
  }
  ...
  function setRouteKey () {
    ...
    for (const _route of _routes) {
      ...
      if(!_route.isChildHide && Array.isArray(_route.children) && _route.children.length > 0) {
            ...
            items={
              (routes.map(
                ({ layout, pages, children }) =>
                  layout === "admin" &&
                  pages
                  .filter(({ isSideBar, access }) => isSideBar 
                    && ( cookies.role === userRoles.SUPER_ADMIN ? true : Array.isArray(access) ? access.includes(cookies.role): false )
                  )
                  .map(({ key, name, icon, path, children: chldrn, showChildren }) => (
                    {
                      key,
                      icon,
                      label: name,
                      onClick: () => !showChildren && navigate(path),
                      children: (!Array.isArray(chldrn) || chldrn.length == 0) || !showChildren ? null : chldrn.map(ch => (
                        {
                          key: ch.key,
                          icon: ch.icon,
                          label: ch.name,
          ...
          <Header style={{ padding: 0, background: colorBgContainer }}>
            <div className='flex items-center justify-between mx-4'>
              <div className='flex items-center justify-start gap-4'>
                ...
                <div>
                  <span className='text-lg font-semibold select-none'>Bids Info Global Panel</span>
                </div>
              </div>
              <div>
                <Dropdown
                ...
                >
                  <Button icon={<UserOutlined />} className='rounded-full p-1'></Button>
                </Dropdown>
              </div>
            </div>
          </Header>
            ...
            <Routes>
              {routes.map(
                ({ layout, pages }) =>
                  layout === "admin" &&
                  pages
                  .filter(({ access }) => ( cookies.role === userRoles.SUPER_ADMIN ? true : Array.isArray(access) ? access.includes(cookies.role): false )
                  )
                  .map(({ path, element, children }) => (
                    <>
                      <Route exact path={path} element={element} />
                      {
                        ...
                        children.map(ch => (
                          <Route exact path={ch.path} element={ch.element} />
                        ))
                      }
                    </>
