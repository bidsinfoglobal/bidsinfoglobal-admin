...
export default function SignIn() {
  ...
  const OnSubmit = async (data) => {
    try {
      ...
      var token = res.result.tokens;
      var roles = res.result.roles;
      setCookie('token', token);
      setCookie('role', roles);
      // navigate("/");
  ...
  <div className="h-full">
    ...
    <div
      className="g-6 flex h-full flex-wrap items-center justify-center lg:justify-between md:mx-8">
      <div
        ...
        <h1 className='font-semibold text-2xl mb-3'>Sign In</h1>
          ...
          <label
            ...
            <input
              style={{ color: 'black' }}
              type="email"
              className="ring-1 ring-gray-500 peer block min-h-[auto] w-full rounded border-0 bg-transparent px-3 py-[0.32rem] leading-[2.15] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none dark:text-neutral-200 dark:placeholder:text-neutral-200 [&:not([data-te-input-placeholder-active])]:placeholder:opacity-0"
              id="exampleFormControlInput2"
              placeholder="Email address" {...register('email')} required />
