        ...
        <div>
            <div className="flex items-center justify-between mb-4">
                ...
                <ManagerModel
                    ...
                    tender_ids={Tender.records.map((r) => r._id)}
                />
            </div>
...
function ManagerModel({ tender_ids, submitHandler, fetchCustomers }) {
    ...
    const onSubmit = async () => {
        if (selectedRowKeys.length === 0) return alert('please select customer');
    ...
    const onSearch = (val) => {
        ...
        var _c = Customer.records.filter(
            (c) => c.full_name.match(val) || c.email.match(val),
        );
        setCust(_c);
    };
