...
import { fetchTenderRecords, insertTenderRecord, updateTenderRecord, deleteTenderRecord, changeTenderParameter } from '../../redux/slice/tender.slice'
  ...
  const [cookies] = useCookies(['role']);
    ...
    useEffect(() => {
      setPagination({
        ...
        fetchRecord({pageNo: Tender.pageNo, limit: Tender.limit, sortBy: Tender.sortBy, sortField: Tender.sortField, keywords: Tender.keywords})
    }, [])
    ...
    function fetchRecord({pageNo, limit, sortBy, sortField, keywords}) {
      dispatch(fetchTenderRecords({ pageNo, limit, sortBy, sortField, keywords: keywords }));
    ...
    .concat([userRoles.SUPER_ADMIN, userRoles.CONTENT_DEPARTMENT].includes(cookies.role) ? {
      title: 'Action',
      dataIndex: 'Action',
      key: 'Action',
      width: '150px',
      ...
      render: (e, record) => <div className="flex gap-2" key={record._id}>
          <ManagerModel key={record._id} id={record._id} submitHandler={handleTenderSubmit} _tender={record} />
...
function ManagerModel({ id, _tender, submitHandler }) {
    ...
    return (
        <div className="">
            {[userRoles.SUPER_ADMIN, userRoles.CONTENT_DEPARTMENT].includes(cookies.role) && <Button className={!id ? 'float-right mb-2' : ''} title={(id ? "Edit" : "Add")} icon={id ? <EditFilled /> : <PlusOutlined />} onClick={() => setOpen(true)}>{id ? '' : 'Add'}</Button>}

            <Modal centered width={1000} title={(id ? "Edit" : "Add") + " Tender"} open={open} onOk={() => setOpen(false)} footer={false} onCancel={() => setOpen(false)}>
                <div>
                    <form onSubmit={handleSubmit(onSubmit)}>
                      <div className='grid md:grid-cols-2 lg:grid-cols-3 md:gap-3'>
                        ...
                        <div className="form-group mb-6">
                            ...
                            <input type="text" className={inputClass} name="authority_name" {...register("authority_name", { required: 'field is required' })}
                                aria-describedby="authority_name" placeholder="authority_name" defaultValue={_tender?.authority_name || ''} />
                        </div>
                        <div className="form-group mb-6">
                            <label className="font-bold">Address</label>
                            <span className='text-red-600 md:ml-4'>{errors?.address?.message}</span>
                            <input type="text" className={inputClass} name="address" {...register("address")}
                                aria-describedby="address" placeholder="address" defaultValue={_tender?.address || ''} />
                        </div>
                        <div className="form-group mb-6">
                            <label className="font-bold">Phone</label>
                            <span className='text-red-600 md:ml-4'>{errors?.telephone?.message}</span>
                            <input type="text" className={inputClass} name="telephone" {...register("telephone")}
                                aria-describedby="telephone" placeholder="telephone" defaultValue={_tender?.telephone || ''} />
                        </div>
                        <div className="form-group mb-6">
                            ...
                            <input type="text" className={inputClass} name="fax_number" {...register("fax_number")}
                                aria-describedby="fax_number" placeholder="fax_number" defaultValue={_tender?.fax_number || ''} />
                        </div>
                        <div className="form-group mb-6">
                            <label className="font-bold">Email</label>
                            <span className='text-red-600 md:ml-4'>{errors?.email?.message}</span>
                            <input type="email" className={inputClass} name="email" {...register("email")}
                                aria-describedby="email" placeholder="email" defaultValue={_tender?.email || ''} />
                        </div>
                        <div className="form-group mb-6">
                            ...
                            <input type="text" className={inputClass} name="contact_person" {...register("contact_person")}
                                aria-describedby="contact_person" placeholder="contact_person" defaultValue={_tender?.contact_person || ''} />
                        </div>
                      </div>
                      <div className='grid md:grid-cols-2 lg:grid-cols-3 md:gap-3'>
                        <div className="form-group mb-6">
                            <label className="font-bold">Big Ref Number <span className='text-red-600'>*</span></label>
                            <span className='text-red-600 md:ml-4'>{errors?.big_ref_no?.message}</span>
                            <input type="text" className={inputClass} name="big_ref_no" {...register("big_ref_no", { required: 'field is required' })}
                                aria-describedby="big_ref_no" placeholder="big_ref_no" defaultValue={_tender?.big_ref_no || ''} />
                        </div>
                        <div className="form-group mb-6">
                            ...
                            <input type="text" className={inputClass} name="description" {...register("description")}
                                aria-describedby="description" placeholder="description" defaultValue={_tender?.description || ''} />
                        </div>
                        <div className="form-group mb-6">
                            <label className="font-bold">Tender Type</label>
                            <span className='text-red-600 md:ml-4'>{errors?.tender_type?.message}</span>
                            <select type="text" className={inputClass} name="tender_type" {...register("tender_type")}
                                aria-describedby="tender_type" placeholder="tender_type" defaultValue={_tender?.tender_type || ''}>
                                  {tenderTypeConst.map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                        </div>
                        <div className="form-group mb-6">
                            <label className="font-bold">Tender No</label>
                            <span className='text-red-600 md:ml-4'>{errors?.tender_no?.message}</span>
                            <input type="text" className={inputClass} name="tender_no" {...register("tender_no")}
                                aria-describedby="tender_no" placeholder="tender_no" defaultValue={_tender?.tender_no || ''} />
                        </div>
                        <div className="form-group mb-6">
                        ...
                        <div className="form-group mb-6">
                            ...
                            <span className='text-red-600 md:ml-4'>{errors?.tender_competition?.message}</span>
                            <select type="text" className={inputClass} name="tender_competition" {...register("tender_competition")}
                                aria-describedby="tender_competition" placeholder="tender_competition" defaultValue={_tender?.tender_competition || ''} >
                                  {competitionType.map(c => <option key={c} value={c}>{c}</option>)}
                              </select>
                        </div>
                      </div>
                      <div className='grid md:grid-cols-2 lg:grid-cols-3 md:gap-3'>
                        <div className="form-group mb-6">
                            ...
                            <span className='text-red-600 md:ml-4'>{errors?.published_date?.message}</span>
                            <input type="date" className={inputClass} name="published_date" {...register("published_date")}
                                aria-describedby="published_date" placeholder="published_date" defaultValue={moment(_tender?.published_date).format('YYYY-MM-DD') || ''} />
                        </div>
                        <div className="form-group mb-6">
                            <label className="font-bold">Closing Date</label>
                            <span className='text-red-600 md:ml-4'>{errors?.closing_date?.message}</span>
                            <input type="date" className={inputClass} name="closing_date" {...register("closing_date")}
                                aria-describedby="closing_date" placeholder="closing_date" defaultValue={moment(_tender?.closing_date).format('YYYY-MM-DD') || ''} />
                        </div>
                        <div className="form-group mb-6">
                        ...
                        <div className="form-group mb-6">
                            ...
                            <span className='text-red-600 md:ml-4'>{errors?.emd?.message}</span>
                            <input type="text" className={inputClass} name="emd" {...register("emd")}
                                aria-describedby="emd" placeholder="emd" defaultValue={_tender?.emd || ''} />
                        </div>
                        <div className="form-group mb-6">
                            <label className="font-bold">Estimated Cost</label>
                            <span className='text-red-600 md:ml-4'>{errors?.estimated_cost?.message}</span>
                            <input type="text" className={inputClass} name="estimated_cost" {...register("estimated_cost")}
                                aria-describedby="estimated_cost" placeholder="estimated_cost" defaultValue={_tender?.estimated_cost || ''} />
                        </div>
                        <div className="form-group mb-6">
                            <label className="font-bold">Documents</label>
                            <span className='text-red-600 md:ml-4'>{errors?.documents?.message}</span>
                            <input type="text" className={inputClass} name="documents" {...register("documents")}
                                aria-describedby="documents" placeholder="documents" defaultValue={_tender?.documents || ''} />
                        </div>
                      </div>
                      <div className='grid md:grid-cols-2 lg:grid-cols-3 md:gap-3'>
                      <div className="form-group mb-6">
                          <label className="font-bold">Sector <span className='text-red-600'>*</span></label>
                          ...
                          <AutoComplete
                            {...register("sectors", { required: 'field is required' })}
                            // className={inputClass}
                            className='w-full'
                      ...
                      <div className="form-group mb-6">
                          <label className="font-bold">Region <span className='text-red-600'>*</span></label>
                          ...
                          <AutoComplete
                              {...register("regions", { required: 'field is reuqired' })}
                              // className={inputClass}
                              className={'w-full'}
                      ...
                      <div className="form-group mb-6">
                          <label className="font-bold">CPV Code <span className='text-red-600'>*</span></label>
                          ...
                          <AutoComplete
                              {...register("cpv_codes", { required: 'field is reuqired' })}
                              // className={inputClass}
                              className={'w-full'}
