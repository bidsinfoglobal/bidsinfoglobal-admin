...
import { fetchProcurmentRecords, updateProcurmentRecord } from '../../../redux/slice/eprocurment.slice';
...
export default function EprocurmentRecordCMS() {
    ...
    const handleDeleteSlide = (id) => {
        dispatch(updateProcurmentRecord({ _id: id, delete: true, label: 'l', icon: 'i' }));
        setTimeout(() => {
            dispatch(fetchProcurmentRecords());
        }, 100);
    }
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (e, record) => <div className="flex gap-2" key={record._id}>
                <EprocurmentModel key={record._id} id={record._id} submitHandler={updateHandler} _service={record} />
  ...
  return (
    <div>
        <div className='flex items-center justify-between mb-4'>
            <h1 className='text-xl md:text-2xl lg:text-3xl font-bold font-bold'>Eprocument Record</h1>
...
function EprocurmentModel({ id, _service, submitHandler }) {
    ...
    return (
        <div className="">
            ...
            <Modal centered title={(id ? "Edit" : "Add") + " Eprocument Record"} open={open} onOk={() => setOpen(false)} footer={false} onCancel={() => setOpen(false)}>
                <div>
                    <form onSubmit={handleSubmit(onSubmit)}>
                        {id && <input type='hidden' name='id' value={id} />}
                        <div className="form-group mb-6">
                            <label className="font-bold">Label <span className='text-red-600'>*</span></label>
                            <span className='text-red-600 md:ml-4'>{errors?.label?.message}</span>
                            <input type="text" className={inputclass} name="label" {...register("label", { required: 'field is required' })}
                                aria-describedby="label" placeholder="label" defaultValue={_service?.label || ''} />
                        </div>
                        <div className="form-group mb-6">
                            ...
                            <input type="text" className={inputclass} name="icon" {...register("icon", { required: 'field is required' })}
                                aria-describedby="icon" placeholder="icon" defaultValue={_service?.icon || ''} />
                        </div>
                        <div>
