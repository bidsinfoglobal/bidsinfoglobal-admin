...
import { fetchCountryRecords, insertCOuntryRecord, updateCountryRecord, deleteCountryRecord, changeCountryParameter } from '../../../redux/slice/country.slice'
...
export default function CountryMaster() {
  ...
  useEffect(() => {
    ...
    fetchRecord({ pageNo: Country.pageNo, limit: Country.limit, sortBy: Country.sortBy, sortField: Country.sortField || "_id", keywords: Country.keywords })
  }, [])
  ...
  const onChange_table = (paginate, filter, sorter, extra) => {
    ...
    dispatch(changeCountryParameter({ pageNo: paginate.current - 1, limit: paginate.pageSize, sortBy: sorter.order == 'ascend' ? 1 : -1, sortField: sorter.field || "_id" }))
    dispatch(fetchCountryRecords({ pageNo: paginate.current - 1, limit: paginate.pageSize, sortBy: sorter.order == 'ascend' ? 1 : -1, sortField: sorter.field || "_id", keywords: Country.keywords }));
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (e, record) => <div className="flex gap-2" key={record._id}>
        <ManagerModel key={record._id} id={record._id} submitHandler={handleCountrySubmit} _country={record} />
...
function ManagerModel({ id, _country, submitHandler }) {
  ...
  return (
    <div className="">
      ...
      <Modal centered title={(id ? "Edit" : "Add") + " Country"} open={open} onOk={() => setOpen(false)} footer={false} onCancel={() => setOpen(false)}>
        <div>
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="form-group mb-6">
              ...
              <input type="text" className={inputClass} name="name" {...register("name", { required: 'field is required' })}
                aria-describedby="name" placeholder="name" defaultValue={_country?.name || ''} />
            </div>
            <div className="form-group mb-6">
              ...
              <input type="text" className={inputClass} name="title" {...register("title", { required: 'field is required' })}
                aria-describedby="description" placeholder="title" defaultValue={_country?.title || ''} />
            </div>
            <div className="form-group mb-6">
              ...
              <textarea type="text" className={inputClass} name="description" {...register("description", { required: 'field is required' })}
                aria-describedby="description" placeholder="description" defaultValue={_country?.description || ''} />
            </div>
            <div className="form-group mb-6">
              <label className="font-bold">Num Code <span className='text-red-600'>*</span></label>
              <span className='text-red-600 md:ml-4'>{errors?.num_code?.message}</span>
              <input type="text" className={inputClass} name="num_code" {...register("num_code", { required: 'field is required' })}
                aria-describedby="num_code" placeholder="num code" defaultValue={_country?.num_code || ''} />
            </div>
            <div className="form-group mb-6">
              <label className="font-bold">Str Code <span className='text-red-600'>*</span></label>
              <span className='text-red-600 md:ml-4'>{errors?.str_code?.message}</span>
              <input type="text" className={inputClass} name="str_code" {...register("str_code", { required: 'field is required' })}
                aria-describedby="str_code" placeholder="str code" defaultValue={_country?.str_code || ''} />
            </div>
            <div>
