...
import { fetchRegionRecords, insertRegionRecord, updateRegionRecord, deleteRegionRecord, changeRegionParameter } from '../../../redux/slice/region.slice'
...
export default function RegionMaster() {
  ...
  useEffect(() => {
    ...
    fetchRecord({ pageNo: Region.pageNo, limit: Region.limit, sortBy: Region.sortBy, sortField: Region.sortField || "_id", keywords: Region.keywords })
  }, [])
  ...
  const onChange_table = (paginate, filter, sorter, extra) => {
    ...
    dispatch(changeRegionParameter({ pageNo: paginate.current - 1, limit: paginate.pageSize, sortBy: sorter.order == 'ascend' ? 1 : -1, sortField: sorter.field || "_id" }))
    dispatch(fetchRegionRecords({ pageNo: paginate.current - 1, limit: paginate.pageSize, sortBy: sorter.order == 'ascend' ? 1 : -1, sortField: sorter.field || "_id", keywords: Region.keywords }));
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (e, record) => <div className="flex gap-2" key={record._id}>
        <ManagerModel key={record._id} id={record._id} submitHandler={handleRegionSubmit} _region={record} />
...
function ManagerModel({ id, _region, submitHandler }) {
  ...
  return (
    <div className="">
      ...
      <Modal centered title={(id ? "Edit" : "Add") + " Region"} open={open} onOk={() => setOpen(false)} footer={false} onCancel={() => setOpen(false)}>
        <div>
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="form-group mb-6">
              ...
              <input type="text" className={inputClass} name="code" {...register("code", { required: 'field is required' })}
                aria-describedby="code" placeholder="code" defaultValue={_region?.code || ''} />
            </div>
            <div>
